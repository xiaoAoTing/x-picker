    ...
    <transition name="slide-picker">
      <div class="x-picker" v-show="modelValue">
        <div class="x-picker__header">
          <p class="x-picker__header-left" @touchstart="handleCancel">取消</p>
          <p class="x-picker__header-center">请选择</p>
          <p class="x-picker__header-right" @touchstart="handleConfirm">确定</p>
        </div>
        <div class="x-picker__content" @wheel="wheelHandle">
          <div class="x-picker__content-wrapper"></div>
          <ul class="x-picker__content-box" ref="box" :style="getOffsetY" @touchstart="handleStart" @touchend="handleEnd"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    let $useList = useList(props)
    const $useEvent = useEvent({ emit, ctx })
    console.log($useList, $useEvent, 'defineComponent')
    let newData = Object.assign({}, $useList);
